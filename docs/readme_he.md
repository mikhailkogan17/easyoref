...
> Guides: [RPi](rpi.md) · [Local](local.md)

### עדכון
כדי לעדכן את המופע המופרס, הריצו:
...
LangGraph `MemorySaver` שומר מצב לפי `alertId`.

Graceful degradation: `ai.enabled: false` — רק שכבת ליבה, בלי LLM בכלל.
